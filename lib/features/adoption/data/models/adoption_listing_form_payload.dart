class AdoptionListingFormPayload {
  ...
  Map<String, dynamic> toApiPayload({required bool submitNow}) {
    ...
    return {
      'petName': _trim(name),
      if (animalTypeId != null) 'animalTypeId': animalTypeId,
      if (breedId != null) 'breedId': breedId,
      if (_trim(gender).isNotEmpty) 'sex': _trim(gender),
      if (_trim(sizeText).isNotEmpty) 'size': _trim(sizeText),
      if (_trim(colorText).isNotEmpty) 'colors': _trim(colorText),
      'story': _trim(description),
      'adoptionReason': _trim(adoptionReason),
      // Always sent, including explicit `null` — this payload represents the
      // full current form state, not a sparse patch. The rural/urban paths
      // are mutually exclusive (e.g. upazila vs. area-only), so switching
      // between them must be able to CLEAR a previously-set field, not just
      // leave it silently stale on the server because the key was omitted.
      'countryId': countryId,
      ...
      if (approximateDateOfBirth != null)
      ...
      if (_customAreas.isNotEmpty) 'customServiceAreas': _customAreas,
      'allowInternationalAdoption': allowInternationalAdoption,
      'vaccinated': vaccinated,
      'dewormed': dewormed,
      'neutered': neutered,
      'microchipped': microchipped,
      if (adopterConditions.isNotEmpty) 'adopterConditions': adopterConditions,
